...
def start_server(server):
    httpd = socketserver.TCPServer(
        (server, PORT), http.server.SimpleHTTPRequestHandler
    )
...
def get_flash_size():
    ...
    if not buf:
        print(
            "Unable to obtain CPU information; make sure to not use A0 stepping!"
        )
...
def image_dump(tn, dumpfile):
    if not dumpfile:
        ...
        if i > 0:
            ...
            sys.exit(1)
        dumpfile = "{}-{}.bin".format(
            platform, buf[i : i + 17].replace(":", "")
        )
    else:
        tn.write("\n")

    print(
        "Dumping flash contents (%dMB) to %s" % (flashsize / 1048576, dumpfile)
    )
    f = open(dumpfile, "wb")
    ...
    for addr in range(t):
        ...
        while 1:
            ...
            if s and s[0][-1] == ":":
                a = int(s[0][:-1], 16)
                if a != count:
                    print(f"Format error: {a:x} != {count:x}")
                    sys.exit(2)
            count += 16
...
try:
    ...
    print(f"Unable to establish telnet session to {HOST}: {msg}")
    sys.exit(1)
