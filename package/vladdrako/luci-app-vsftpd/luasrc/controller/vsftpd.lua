...
#	entry({"admin", "services"}, firstchild(), "NAS", 44).dependent = false
	...
	entry({"admin", "services", "vsftpd", "users"}, cbi("vsftpd/users"), _("Virtual Users"), 20).leaf = true 
	entry({"admin", "services", "vsftpd", "anonymous"}, cbi("vsftpd/anonymous"), _("Anonymous User"), 30).leaf = true
	entry({"admin", "services", "vsftpd", "log"}, cbi("vsftpd/log"), _("Log Settings"), 40).leaf = true
	entry({"admin", "services", "vsftpd", "item"}, cbi("vsftpd/item"), nil).leaf = true
end
