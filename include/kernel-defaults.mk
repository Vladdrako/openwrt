...
define Kernel/CompileImage/Initramfs
	$(call locked,{ \
		...
		$(call Kernel/Configure/Initramfs,$(1),$(LINUX_DIR)$(2)); \
		...
		$(if $(CONFIG_TARGET_ROOTFS_INITRAMFS_SEPARATE), \
			...
			$(if $(SOURCE_DATE_EPOCH), \
				touch -hcd "@$(SOURCE_DATE_EPOCH)" $(KERNEL_BUILD_DIR)/initrd.cpio; \)
			$(if $(CONFIG_TARGET_INITRAMFS_COMPRESSION_BZIP2), \
				$(STAGING_DIR_HOST)/bin/bzip2 -9 -c < $(KERNEL_BUILD_DIR)/initrd.cpio > $(KERNEL_BUILD_DIR)/initrd.cpio.bzip2; \)
			$(if $(CONFIG_TARGET_INITRAMFS_COMPRESSION_GZIP), \
				$(STAGING_DIR_HOST)/bin/libdeflate-gzip -n -f -S .gzip -12 $(KERNEL_BUILD_DIR)/initrd.cpio; \)
			$(if $(CONFIG_TARGET_INITRAMFS_COMPRESSION_LZ4), \
				$(STAGING_DIR_HOST)/bin/lz4c -l -c1 -fz --favor-decSpeed $(KERNEL_BUILD_DIR)/initrd.cpio; \)
			...
			$(if $(CONFIG_TARGET_INITRAMFS_COMPRESSION_LZO), \
				$(STAGING_DIR_HOST)/bin/lzop -9 -f $(KERNEL_BUILD_DIR)/initrd.cpio; \)
			...
			$(if $(CONFIG_TARGET_INITRAMFS_COMPRESSION_ZSTD), \
				$(STAGING_DIR_HOST)/bin/zstd -T0 -f -o $(KERNEL_BUILD_DIR)/initrd.cpio.zstd $(KERNEL_BUILD_DIR)/initrd.cpio; \)
