...
class DownloadGitHubError(Exception):
    pass
...
class Path:
    ...
    def untar(path, into=None):
        ...
        if len(dirs) == 1:
            ...
            raise PathException(
                f"untar {path}: expecting a single subdir, got {dirs}"
            )
...
class DownloadGitHubTarball:
        ...
        with Path(TMPDIR_DL, keep=True) as dir_dl:
            ...
            with Path(tarball_path, isdir=False):
                ...
                with Path(d, preclean=True) as dir_untar:
                    tarball_prefix = Path.untar(
                        tarball_path, into=dir_untar.path
                    )
    ...
    def _hash_check(self, f):
        ...
        if xhash != self.xhash:
            raise self._error(
                "Wrong hash (probably caused by .gitattributes), expecting {}, got {}".format(
                    self.xhash, xhash
                )
            )
