...
def decode_header(datafile):
	...
	for field in range(header['num_items']):
...
def extract(datafile):
	...
	for section in header['items']:
		...
		with open(f"{section['name']}.bin", 'wb') as f:
...
def get_section_contents(section):
	if section.get('data'):
		...
		with open(section['file'], 'rb') as sf:
			data = sf.read()
	else:
		data = bytes()
...
def compress(output_image, header):
	with open(output_image, 'w+b') as f:
		# header MD5
		...
		for section in header['items']:
			f.seek(0x1814 + section['offset'])
			f.write(get_section_contents(section))

		size = f.tell()

		f.seek(4)
		md5_sum = hashlib.md5(f.read())

		f.seek(0)
		f.write(struct.pack('>I16s', size, md5_sum.digest()))
...
def create_image(output_image, root, support):
	...
	compress(output_image, header)

def main(args):
	if args.extract:
		with open(args.image, 'rb') as image:
			header = extract(image)
	elif args.create:
